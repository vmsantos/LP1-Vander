...
inverteReceituarioAux :: (Medicamento, [Horario]) -> [(Horario, Medicamento)]
inverteReceituarioAux (_, []) = []
inverteReceituarioAux (m, h : tail) = (h, m) : (inverteReceituarioAux (m, tail))

inverteReceituario :: Receituario -> [(Horario, Medicamento)]
inverteReceituario [] = []
inverteReceituario (h : tail) = quickSort (((inverteReceituarioAux h)) ++ (inverteReceituario tail)) 

percorreLista :: [Horario] -> Receituario -> PlanoMedicamento 
percorreLista [] _ = []
percorreLista (h:hs) r = (h,retornaMedHorario h (inverteReceituario r)) : percorreLista hs r
...
invertePlanoAux :: (Horario, [Medicamento]) -> [(Medicamento, Horario)]
invertePlanoAux (_, []) = []
invertePlanoAux (m, h : tail) = (h, m) : (invertePlanoAux (m, tail))

invertePlano :: PlanoMedicamento -> [(Medicamento, Horario)]
invertePlano [] = []
invertePlano (h : tail) = quickSort (((invertePlanoAux h)) ++ (invertePlano tail)) 

percorreLista2 :: [Medicamento] -> PlanoMedicamento -> Receituario 
percorreLista2 [] _ = []
percorreLista2 (h:hs) p = (h,retornaHorMedicamento h (invertePlano p)) : percorreLista2 hs p
...
geraCompras [] _ = []
geraCompras ((m,q):ds) e 
  | (q - (consultarMedicamento m e)) > 0 = (Comprar m (q - (consultarMedicamento m e))) : geraCompras ds e
  | otherwise = geraCompras ds e
